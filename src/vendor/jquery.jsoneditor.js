...
var _jqueryJsoneditor = require("./jquery.jsoneditor.menu");

var _actionIO = require("../app/action/actionIO");

var actionIO = _interopRequireWildcard(_actionIO);

var _componentIO = require("../app/component/componentIO");

var componentIO = _interopRequireWildcard(_componentIO);

function _interopRequireWildcard(obj) { if (obj && obj.__esModule) { return obj; } else { var newObj = {}; if (obj != null) { for (var key in obj) { if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key]; } } newObj.default = obj; return newObj; } }

var from = void 0; // Simple yet flexible JSON editor plugin.
...
items.push({
    ...
    submenu: [{
        ...
        title: "titles.array",
        click: function click(button, opt) {
...
function jsonEditor(container, json, options, caller) {
    from = caller;
    options = options || {};
...
function feed(o, path, value) {
    ...
    if (path.indexOf('.') > -1) {
        ...
        if (del) delete diver[parts[len - 1]];else diver[parts[len - 1]] = value;
    } else {
        if (del) delete o[path];else o[path] = value;
    }
    return o;
}
...
function addMenu(item, opt) {
    if (item.children('.menuer').length == 0) {
        ...
        menuer.bind('click', function () {
            ...
            (0, _jqueryJsoneditor.ContextMenu)($(this), items, opt);
        });
        item.prepend(menuer);
    }
}
...
function changeType(type, button, opt) {
    ...
    if (type == "string") value.val('""');else if (type == "array") value.val("[]");else if (type == "object") value.val("{}");
...
function assignType(item, val) {
    ...
    if (isObject(val)) className = 'object';else if (isArray(val)) className = 'array';else if (isBoolean(val)) className = 'boolean';else if (isString(val)) className = 'string';else if (isNumber(val)) className = 'number';
