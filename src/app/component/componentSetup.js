...
exports.initComponentSetup = initComponentSetup;

var _componentSetupData = require("./componentSetupData");

var componentSetupData = _interopRequireWildcard(_componentSetupData);
...
function initComponentSetup(component) {
...
function showPorts() {
    ...
    _.each(componentSetupData.data.service.spec.ports, function (item, index) {
        var row = '<div class="port-row"><div class="port-div">' + '<div>' + '<label for="normal-field" class="col-sm-4 control-label">' + 'Target Port' + '</label>' + '<div class="col-sm-7" data-index="' + index + '">' + '<input type="number" name="k8s-service-port" value="' + item.port + '" class="form-control" required min="0" max="65535">' + '</div>' + '</div>' + '</div>' + '<div class="target-port-div">' + '<div>' + '<label for="normal-field" class="col-sm-4 control-label">' + 'Node Port' + '</label>' + '<div class="col-sm-7" data-index="' + index + '">' + '<input type="number" name="k8s-service-node-port" value="' + item.nodePort + '" class="form-control" required min="0" max="65535">' + '</div>' + '</div>' + '</div>' + '<div class="port-remove-div rm-port" data-index="' + index + '">' + '<span class="glyphicon glyphicon-remove"></span>' + '</div></div>';
...
function toJsonYaml(type) {
    ...
    try {
        ...
        try {
            ...
            (0, _notify.notify)("Your advanced " + type + " setting is not a legal json or yaml.", "error");
            result = false;
        }
    }
    if (!result) {
