...
$(function () {
    var ScaffoldAppView = function ScaffoldAppView() {
    ...
    ScaffoldAppView.prototype.expandNavigation = function () {
        ...
        this.$sidebar.find('.active .active').closest('.collapse').collapse('show').siblings('[data-toggle=collapse]').removeClass('collapsed');
    };
    ...
    ScaffoldAppView.prototype.log = function (message) {
        if (this.debug) {
            console.log("ScaffoldApp: " + message + " - " + arguments.callee.caller.toString().slice(0, 30).split('\n')[0] + " - " + this.extractPageName(location.href));
        }
    };
...
function initAppPlugins() {
    ...
    !function ($) {
        $(document).on('click', 'table th [data-check-all]', function () {
            $(this).closest('table').find('input[type=checkbox]').not(this).prop('checked', $(this).prop('checked'));
        });
    }(jQuery);
    ...
    !function ($) {
        ...
        $('#nav-state-toggle').onPositionChanged(function () {
            Tether.position();
        }, 0);
        $('#nav-collapse-toggle').onPositionChanged(function () {
            Tether.position();
        }, 0);
    }(jQuery);
}
    ...
    !function ($) {
        ...
        $(document).on('click', '.chat-sidebar-user-group > a', function () {
            ...
            $('.message-list', $target).slimscroll({
                height: $target.height() - $targetTitle.height() - parseInt($targetTitle.css('margin-top')) - parseInt($targetTitle.css('margin-bottom')),
                width: '',
                size: '4px'
            });
            return false;
        ...
        $('#chat-sidebar-input').keyup(function (e) {
            ...
            var $currentMessageList = $('.chat-sidebar-chat.open .message-list'),
                $message = $('<li class="message from-me">' + '<span class="thumb-sm"><img class="img-circle" src="img/avatar.png" alt="..."></span>' + '<div class="message-body"></div>' + '</li>');
...
function initDemoFunctions() {
    !function ($) {
        ...
        $('[data-toggle="chat-sidebar"]').one('click', function () {
            setTimeout(function () {
                $('.chat-sidebar-user-group:first-of-type .list-group-item:first-child').addClass('active').find('.fa-circle').after('<span class="label label-pill label-danger pull-xs-right animated bounceInDown">3</span>');
            }, 1000);
        });

        setTimeout(function () {
            var $chatNotification = $('#chat-notification');
            $chatNotification.removeClass('hide').addClass('animated fadeIn').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                $chatNotification.removeClass('animated fadeIn');
                setTimeout(function () {
                    $chatNotification.addClass('animated fadeOut').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
